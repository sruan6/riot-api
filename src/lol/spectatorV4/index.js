...
const Spectator = function Spectator(region, apikey) {
  this.activeGamesById = async (Id) => {
    const { data } = await axios.get(
      `https://${region[0]}/lol/spectator/v4/active-games/by-summoner/${Id}?api_key=${apikey}`,
    );
    return data;
  };
  ...
  this.featureGames = async () => {
    const { data } = await axios.get(
      `https://${region[0]}/lol/spectator/v4/featured-games?api_key=${apikey}`,
    );
    return data;
  };
};
